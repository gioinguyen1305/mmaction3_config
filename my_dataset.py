...
class MyDataset(BaseActionDataset):
    ...
    def __init__(self,
                 ...
                 filename_tmpl: str = 'frame_{:06}.PNG',
                 start_index: int = 0,
        ...
        if self.proposal_file is not None:
            self.proposals = pd.read_csv(self.proposal_file, header=None)
        else:
            self.proposals = None
            self.proposals = None
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for line in fin:
            ...
            self.timestamp_end = len(glob.glob(self.data_prefix["img"] + "/" + video_id + "/*"))
            ...
            if self.use_frames:
                ...
                #              self._FPS)
                shot_info = (0, self.timestamp_end-1)
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        data_info['total_frames'] = len(glob.glob(data_info["frame_dir"] + "/*")) - 1
        if self.proposals is not None:
            # pps = self.proposals[(self.proposals[0]== int(data_info["video_id"])) & (self.proposals[1].isin([data_info["timestamp"] - 1, data_info["timestamp"], data_info["timestamp"] +1]))]
            pps = self.proposals[(self.proposals[0]== data_info["video_id"]) & (self.proposals[1] == data_info["timestamp"])]
            if len(pps) == 0:
                data_info['proposals'] = np.array([[0, 0, 1, 1]])
                data_info['scores'] = np.array([1])
            else:
                proposals = pps.iloc[:,2:].values
                ...
                if proposals.shape[-1] == 5:
                    ...
                    data_info['scores'] = np.array([1]*len(proposals))
                else:
                    proposals = proposals[:self.num_max_proposals]
            ...
            data_info['proposals'] = ann['gt_bboxes']
        return data_info
